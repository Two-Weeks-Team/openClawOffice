...
- 각 알림 시그널은 `dedupeKey`를 가지며, UI는 `key={signal.dedupeKey}` 기반 선언형 렌더링으로
  동일 조건의 중복 항목을 하나로 유지합니다.
- 같은 조건이 유지되는 동안에는 동일 키 토스트/카드가 하나만 보이고,
  조건이 바뀌어 `dedupeKey`가 달라질 때 새 항목이 표시됩니다.
